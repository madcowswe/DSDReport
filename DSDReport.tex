...
\begin{abstract}
	...
	This report describes the design considerations and implementation details of the Digital System Design assignment involving two subprojects.
	...
	The design process including topology consideraton and a quantisation analysis is presented, and the detailed hardware implentation is described.
	Further, a considrable timing analysis and several otpimisations to maximise perfomance are presented, and a performance and resouce utilisation analasys is performed. 
...
Seen in Table~\ref{tab:fp_hw_setup} is the configuration of the system during testing. The performance results and the resource usage can be seen in Tables~\ref{tab:sw_perf} and \ref{tab:sw_resourceuse} respectively.
...
There are several different versions of these blocks available, with different clock cycle latency and different Fmax.
We optimise our design decisions around having the smallest area possible block, while maintaining an Fmax that is at least as fast as the target system frequency, which is 160 MHz.
As seen in Table~\ref{tab:fp_unit_bench}, we benchmark the blocks to find their Fmax when actually attached to our system. As we try to pick the smallest possible blocks that meed the speed requirement, the set of blocks are not exhaustively tested.
