...
%In general, if there are spare multiplier blocks, it is better in terms of resource utilisation to use these as they cannot be used for other purposes.
...
The resource usage for the determinant accelerator module is shown in table \ref{tab:resource_util_det}. Note that the big LE usage is due to row/col preloading memory synthesised to LEs instead of being packed into one m9k (which is just a synthesis setting).

\begin{table}[tbp]
	\caption{Resource utilisation of the determinant module. Note that two fp\_mul units are used. Also note that the big LE usage is due to row/col preloading memory synthesised to LEs instead of being packed into one m9k (which is just a synthesis setting)}
